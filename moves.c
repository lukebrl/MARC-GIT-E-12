...
t_move *removeMoveAtIndex(t_move *moves_list, int nb_moves, int index) {
    t_move *new_moves = malloc(sizeof(t_move) * nb_moves);
    if (new_moves == NULL) {
        fprintf(stderr, "Memory allocation failed with new moves_list\n");
        return 0;
    }
    int idx = 0;
    for (int i=0; i < nb_moves; i++) {
        //don't add the element at given index
        if (i == index) {
            continue;
        }

        new_moves[idx] = moves_list[i];
        idx++;
    }
